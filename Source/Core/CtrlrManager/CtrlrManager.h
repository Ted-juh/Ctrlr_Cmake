                        ...
                        public ActionBroadcaster
{
	public:
		...
		class Listener
		{
			public:
				virtual ~Listener(){}
				// The listener will be informed of manager state changes, the new state is passed as the first parameter
		...
		*/
		...
		// Show panel id the DocumentManager (default UI) @param panelToAdd a CtrlrPanelEditor object to show
		...
		// Get a pointer to a panel based on it's name	@param panelName	the name of the panel to return, 0 if not found
		CtrlrPanel *getPanel(const String &panelName);

		// Returns a reference to the internal window manager. The window manager keeps track of all non-modal windows in Ctrlr
		...
		bool canCloseWindow();
		void removeModulator (CtrlrModulator *modulatorToDelete);		// Remove a modulator from the manager
		void restoreState(const XmlElement &savedState);				// Restore state
		void restoreState (const ValueTree &savedTree);					// Restore state 

		// Returns a reference to the manager tree, the root of all in Ctrlr, be careful with this one	@see getManagerTree
		ValueTree &getManagerTree()	{return (managerTree);}
		int getNumModulators(const bool onlyVstParameters);				// Return the number of all modulators available
		CtrlrModulator *getModulatorByVstIndex(const int index); 		// Return modulator based on it's vstIndex
		CtrlrProcessor *owner;											// A pointer to the AudioProcessor class
